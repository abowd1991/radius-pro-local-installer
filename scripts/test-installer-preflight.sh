...
grep -Fq 'database migrations did not create Winbox V2 remote-management tables' "$ROOT_DIR/install.sh"
grep -Fq 'readonly INSTALLER_REF="${RADIUS_PRO_INSTALLER_REF:-v3.3.15}"' "$ROOT_DIR/install.sh"
grep -Fq 'local build_swap_file=""' "$ROOT_DIR/install.sh"
grep -Fq 'local swap_mb' "$ROOT_DIR/install.sh"
grep -Fq 'local build_swap_mb=$((2048 - swap_mb))' "$ROOT_DIR/install.sh"
grep -Fq 'fallocate -l "${build_swap_mb}M" "$build_swap_file"' "$ROOT_DIR/install.sh"
grep -Fq 'NODE_OPTIONS="${NODE_OPTIONS:---max-old-space-size=1024}" pnpm build' "$ROOT_DIR/install.sh"
grep -Fq 'temporary ${build_swap_mb} MiB build swap enabled for low-memory host' "$ROOT_DIR/install.sh"
grep -Fq 'local nginx_stream_package="libnginx-mod-stream"' "$ROOT_DIR/install.sh"
grep -Fq 'radius-pro-remote-management-v2.conf' "$ROOT_DIR/install.sh"
...
grep -Fq 'group_membership_query = "SELECT groupname FROM \${usergroup_table} WHERE username = '\''%{SQL-User-Name}'\'' ORDER BY priority"' "$ROOT_DIR/install.sh"
...
test -f "$ROOT_DIR/app/drizzle/0114_remote_management_access_v2.sql"
test -f "$ROOT_DIR/app/drizzle/0115_remote_management_access_events.sql"
grep -Fq 'CREATE TABLE `remote_management_accesses`' "$ROOT_DIR/app/drizzle/0114_remote_management_access_v2.sql"
grep -Fq 'CREATE TABLE `remote_management_access_events`' "$ROOT_DIR/app/drizzle/0115_remote_management_access_events.sql"
grep -Fq 'remoteManagementAccesses' "$ROOT_DIR/app/drizzle/schema.ts"
grep -Fq 'remoteManagementAccessEvents' "$ROOT_DIR/app/drizzle/schema.ts"
grep -Fq 'remoteManagementAccessRouter' "$ROOT_DIR/app/server/routers.ts"
test -f "$ROOT_DIR/app/server/services/remoteManagementVpsService.ts"
test -f "$ROOT_DIR/app/server/domains/network/RemoteManagementActivationEngine.ts"
grep -Fq "/api/remote-management/v2/sync" "$ROOT_DIR/services/vpn-api.py"
grep -Fq "Winbox V2 requires restricted access mode" "$ROOT_DIR/services/vpn-api.py"
grep -Fq 'remote-management-v2-api' "$ROOT_DIR/scripts/verify-install.sh"
grep -Fq 'remote-management-v2-schema' "$ROOT_DIR/scripts/verify-install.sh"
