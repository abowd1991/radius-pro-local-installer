...
grep -Fq 'readonly INSTALLER_REF="${RADIUS_PRO_INSTALLER_REF:-v3.3.14}"' "$ROOT_DIR/install.sh"
grep -Fq 'local build_swap_file=""' "$ROOT_DIR/install.sh"
grep -Fq 'fallocate -l 1G "$build_swap_file"' "$ROOT_DIR/install.sh"
grep -Fq 'NODE_OPTIONS="${NODE_OPTIONS:---max-old-space-size=1024}" pnpm build' "$ROOT_DIR/install.sh"
grep -Fq 'temporary build swap enabled for low-memory host' "$ROOT_DIR/install.sh"
